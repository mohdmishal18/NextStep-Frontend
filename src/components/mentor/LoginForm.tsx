...
import { googleAuthLogin } from "../../api/mentor";
  ...
  const onSubmit = async (data: LoginFormData) => {
    try {
      ...
      if (
        ...
        dispatch(mentorLogin(response.data.user));
  ...
  const googleLogin = async (response: CredentialResponse) => {
    try {
      ...
      if (
        ...
        dispatch(mentorLogin(googleLoginResponse.data.loginUser));
        navigate("/mentor");
      }
    } catch (error) {
      if (axios.isAxiosError(error)) {
